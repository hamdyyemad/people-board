...
import { GenericListCard } from "./generic-cards";

interface ListDisplayProps<TData> {
  rows: Row<TData>[];
  renderItem?: ((data: TData) => React.ReactNode) | CardConfig<TData>;
  onRowClick?: (data: TData) => void;
}

export function ListDisplay<TData>({
  rows,
  renderItem,
  onRowClick,
}: ListDisplayProps<TData>) {
  if (!rows.length) {
    return (
  ...
  return (
    <div className="flex flex-col gap-2">
      {rows.map((row) => (
        ...
        >
          {isConfig ? (
            <GenericListCard row={row.original} config={renderItem as CardConfig<TData>} />
          ) : (
